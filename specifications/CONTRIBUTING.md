...
The following rules apply to every translations directory in the repository (e.g., `credential-definitions/<credential-type><version>/translations/`, `credential-definitions/<credential-type><version>/input-fields/translations/` or `resources/user-consent/translations`):
...
- All input fields defined for a credential must be mapped in `credential-definitions/<credential-type><version>/user-consent/user-consent-map.json`.
