...
- For domain experts — documentation in progress
- For solution architects — documentation in progress
- For developers — documentation in progress

## Repository Structure

The repository is organised into credential type directories, shared resources, and autogenerated files.

- **[`Credential Definitions`](credential-definitions)** –  A collection of credential-type folders, where each directory represents a specific credential type and contains its schemas, examples, and supporting resources.
  See the [credential type structure specification](specifications/credential-type-structure.md) for a detailed breakdown of expected files and layout.
- **[`Resources`](resources)** – Common assets such as user consent resources used across credential types.
- **[`.autogen`](.autogen)** – Contains autogenerated files representing the currently supported credential types and aggregated repository data.
