...
- **[`_autogen`](_autogen)** – Contains autogenerated files representing the currently supported credential types and aggregated repository data.
- **Credential type folders** – Each directory represents a specific credential type and contains its schemas, examples, and supporting resources.  
  See the [credential type structure documentation](resources/documentation/credential-type-structure.md) for a detailed breakdown of expected files and layout.
- **[`resources`](resources)** – Common assets such as documentation and user consent resources used across credential types.
